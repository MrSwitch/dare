...
/**
 * Patch - MSSQL compatibility for JSON_MERGE_PATCH setFunction usage
 * SQL Server has JSON_MODIFY but not JSON_MERGE_PATCH, so emulate merge-patch in JS
 * for matching rows and then issue regular patch operations per row.
 * @type {Dare['patch']}
 */
MSSQLDare.prototype.patch = async function patch(
	table,
	filter,
	body,
	options = {}
	...
	const modelSchema = dare.options.models?.[opts.table]?.schema || {};

	const jsonMergeFields = Object.entries(opts.body || {})
		...
		.filter(([field]) => {
			...
			return (
				...
				fieldDef.type === 'json' &&
				fieldDef.patch?.setFunction
			);
		})
		.map(([field]) => field);

	if (!jsonMergeFields.length) {
		return Dare.prototype.patch.call(this, opts);
	}

	const fields = [{id: this.rowid}, ...jsonMergeFields];
	const rows = await dare.get({
		table: opts.table,
		fields,
		filter: opts.filter,
		start: opts.start,
	...
	if (!list.length) {
		return opts.notfound ?? {affectedRows: 0};
	}

	const models = {...dare.options.models};
	const model = {...(models[opts.table] || {})};
	const schema = {...(model.schema || {})};

	for (const field of jsonMergeFields) {
		const fieldDef = schema[field];
		const patchDef = {...(fieldDef.patch || {})};
		patchDef.setFunction = null;
		schema[field] = {...fieldDef, patch: patchDef};
	}

	model.schema = schema;
	models[opts.table] = model;

	const patchInstance = dare.use({models});

	let affectedRows = 0;
	for (const row of list) {
		const mergedBody = {...opts.body};
		...
		// eslint-disable-next-line no-await-in-loop
		const patched = await Dare.prototype.patch.call(patchInstance, {
			table: opts.table,
			filter: {[this.rowid]: row.id},
			body: mergedBody,
			notfound: {},
		});
